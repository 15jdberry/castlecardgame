...
""")
    ...
    def reset_game(self):
        print(f"Resetting Game")  # TOGGLE TEST
        Game.deck = [i for i in range(1, 53)]
        print(f"Reset Deck: {Game.deck}")  # TOGGLE TEST
        Game.burnt = []
        Game.stack = []
        Game.active_card = int()
        self.player = Player()
        print(f"player's new hand cards: {self.player.hand}")  # TOGGLE TEST
        self.opp = Opponent()
    ...
    def player_turn(self) -> classmethod:
        ...
        if len(self.player.hand) == 0 and len(self.player.face_up) > 0:
            ...
            candidate_card = self.player.face_up[player_selection - 1]  # WIP TEST RIGHT CARD NUMBER
            ...
            print('below_card_value: ', below_card_value)  # TOGGLE TEST
    ...
    def opponent_turn(self) -> classmethod:
        ...
        # print(f"\tOpponent hand: {self.opp.hand}\n{Utility.id_card_list(self.opp.hand)}")  # TOGGLE TEST
        sleep(0.5)
        Game.active_card = Game.stack[-1] if len(Game.stack) > 0 else int(0)
        if len(self.opp.hand) == 0 and len(self.opp.face_up) > 0:
            ...
            # print('op choice: ', opponent_choice)  # TOGGLE TEST
            # print(f'\tWIP Hidden cards Endgame, active card: {Game.active_card}')  # TOGGLE TEST
            Game.stack.append(opponent_choice)
            self.opp.hidden.remove(opponent_choice)
            # Game.active_card = Game.stack[-1]  # WIP
            ...
            if Utility.get_card_val(opponent_choice) == 8:
                ...
                return self.opponent_turn()
            below_card_value = Utility.get_card_val(Game.stack[-2]) if len(Game.stack) >= 2 else int(0)
            print('WIP - below_card_value: ', below_card_value)  # WIP
    ...
    def id_opponent_playable(self, cardset: str) -> int or classmethod:
        ...
        Game.active_card = Game.stack[-1] if len(Game.stack) > 0 else int(0)
        # print('id_opponent_playable: active card is ', Game.active_card)  # WIP
        print(f"Hand: {Utility.id_card_list(self.opp.hand)}")  # TOGGLE TEST TO SEE OP HAND
        if cardset == 'hidden':
            ...
            # if len(self.opp.hidden) == 1:
            #     # ! Only card in Hidden cards
            #     _candidate_card = self.opp.hidden[0]
            #     return _candidate_card
            # else:
            #     # ! Otherwise, play a random Hidden card
            #     _random_index = random.randint(0, len(self.opp.hidden))
            #     print(f"random index: {_random_index}")  # WIP remove
            #     _candidate_card = self.opp.hidden[_random_index - 1]
            #     print(f"WIP endgame candidate card: {_candidate_card}")  # WIP
            #     return _candidate_card
        ...
        if len(_opponentOptions) == 0:
            ...
            print(f"Opponent options: {_opponentOptions} / {Utility.id_card_list(_opponentOptions)}")  # TOGGLE TEST
            ...
            if len(_opponentOptionsSame) > 0:
                ...
                print(f'\t{self.opp.text_color}Opponent has the n+2 card...{ColorClass.ENDC}')
                _candidate_card = _opponentOptionsN2[0]
            elif len(_opponentOptionsN3) > 0:
                print(f'\t{self.opp.text_color}Opponent has the n+3 card...{ColorClass.ENDC}')
                _candidate_card = _opponentOptionsN3[0]
            # Keep adding for more refined logic
            else:
                _candidate_card = _opponentOptions[0]
            print('chosen _candidate_card:', _candidate_card)  # TOGGLE TEST
            return _candidate_card
...
\tPlease try again or type 99 to pick up the stack.{ColorClass.ENDC}""")
    ...
    def fill_hand(self, turn: str) -> None:
        ...
        """
        sleep(0.2)
        print(f"random deck now: {Game.deck}")  # TOGGLE TEST
        if turn == 'player':
            ...
            if len(self.opp.hand) < 3 and len(Game.deck) > 0:
                ...
                print(f"New Hand: {Utility.id_card_list(self.opp.hand)}")  # TOGGLE TEST
                self.fill_hand(turn)
    ...
    def burn_stack(self, turn: str) -> None:
        ...
        while len(Game.stack) > 0:
            ...
            sleep(0.1)
        print(f"\tBurnt cards: {Game.burnt} / {Utility.id_card_list(Game.burnt)}")  # TOGGLE TEST
        print(f"\tCurrent Stack: {Game.stack}")  # TOGGLE TEST
        self.fill_hand(turn)
    ...
    def pickup_stack(self, turn: str) -> None:
        ...
        if turn == 'player':
            ...
            print(f"\tPicked up stack, New Hand: {Utility.id_card_list(self.opp.hand)}")  # TOGGLE TEST
...
{ColorClass.BOLD}{ColorClass.OKCYAN}{ColorClass.CBLACKBG}└{'─' * 64} deck: {len(Game.deck)} ┘{ColorClass.ENDC}""")
    ...
    def run_game_loop(self, ask_rules: bool = False, keep_record: bool = False) -> classmethod:
        """
